...
const UPLOAD_DIR = path_1.default.join(__dirname, '..', '..', 'public', 'uploads', 'consignments');
...
router.post('/', upload.array('images', 5), async (req, res) => {
    const { buyerId, consignmentType, category, quantity, expectedPrice, description, minPriceType, commissionRate, commissionNegotiable, } = req.body;
    ...
    try {
        ...
        const [result] = await conn.query(`INSERT INTO consignments
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            await conn.query('INSERT INTO consignment_images (consignment_id, image_url, is_primary, display_order) VALUES (?, ?, ?, ?)', [consignmentId, imageUrl, i === 0 ? 1 : 0, i]);
        }
        await conn.commit();
...
router.get('/:id', async (req, res) => {
    ...
    try {
       ...
       WHERE c.id = ?`, [id]);
        ...
        const [imgRows] = await mysql_1.default.query('SELECT image_url, is_primary, display_order FROM consignment_images WHERE consignment_id = ? ORDER BY display_order ASC', [id]);
        const r = rows[0];
        res.json({
            id: r.id,
            buyerId: r.buyer_id,
            ...
            buyerName: r.buyer_name || '익명',
            buyerAvatar: r.buyer_avatar ?? null,
            images: imgRows.map((i) => i.image_url),
        });
    }
    catch (err) {
...
router.post('/:id/match', async (req, res) => {
    ...
    try {
        const [rows] = await mysql_1.default.query('SELECT buyer_id, status FROM consignments WHERE id = ?', [consignmentId]);
        if (!rows.length)
            return res.status(404).json({ error: 'not_found' });
        if (rows[0].status !== 'pending')
            return res.status(409).json({ error: 'already_matched' });
       ...
       WHERE cr.is_dm = 1 LIMIT 1`, [sellerId, buyerId]);
        let roomId;
        if (existing.length > 0) {
            ...
            try {
                await conn.beginTransaction();
                const [r] = await conn.query('INSERT INTO chat_rooms (name, is_dm, created_by) VALUES (?, 1, ?)', ['dm', sellerId]);
                roomId = r.insertId;
