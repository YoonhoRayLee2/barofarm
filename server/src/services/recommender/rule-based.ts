import type { Recommender, AuctionWin, Recommendation, Product, PriceTier } from './types';
import { getMallProducts } from './catalog';
...
// ── 가격 티어 분류 ────────────────────────────────────────────
function classifyPriceTier(price: number): PriceTier {
...
function tierDistance(a: PriceTier, b: PriceTier): number {
...
// ── 핵심 키워드 추출 (조사·단위 제거) ────────────────────────
function extractCoreKeyword(itemName: string): string {
  // "갈치 1박스" → "갈치", "한우 등심 500g" → "한우"
  return itemName.trim().split(/\s+/)[0] ?? itemName;
}

// ── 카테고리 신호 점수 (0|0.3|0.6|1.0) ───────────────────────
function categoryScore(win: AuctionWin, product: Product): { score: number; matchedKw: string[] } {
...
// ── 가격 신호 점수 ────────────────────────────────────────────
function priceScore(win: AuctionWin, product: Product): number {
...
// ── 콘텐츠 유사도 점수 ────────────────────────────────────────
function contentScore(win: AuctionWin, product: Product): { score: number; commonTokens: string[] } {
  ...
  for (const tok of srcTokens) {
    if (dstTokens.has(tok)) commonTokens.push(tok);
  }
...
export class RuleBasedRecommender implements Recommender {
  async recommend(win: AuctionWin, topN: number): Promise<Recommendation[]> {
    const products = getMallProducts();
    const winTier = classifyPriceTier(win.finalPrice);

    // 1차: 각 상품 점수 계산
    type Scored = { product: Product; raw: number; reasons: string[] };
    const scored: Scored[] = products.map(product => {
      ...
      if (pScore > 0) {
        const tierLabel: Record<PriceTier, string> = { low: '소액', mid: '중간', high: '고가' };
