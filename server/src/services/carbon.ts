...
function getCoords(zipcode: string): [number, number] | null {
...
export interface CarbonResult {
  distanceKm: number;
  savedKm: number;
  savedCo2g: number;    // 그램 단위
  savedPct: number;     // 마트 대비 절감률 (%)
}

export function calcCarbon(farmZipcode: string, buyerZipcode: string): CarbonResult | null {
  const farmCoords = getCoords(farmZipcode);
  const buyerCoords = getCoords(buyerZipcode);
  if (!farmCoords || !buyerCoords) return null;
