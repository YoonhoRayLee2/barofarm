...
router.get('/api/settlements/:id', requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [auctions] = await pool.query<any[]>(
...
router.get('/api/sellers', requireAdmin, async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.query<any[]>(
      "SELECT id, nickname FROM users WHERE is_seller = 1 OR role = 'seller' ORDER BY nickname",
    );
    res.json(rows);
  } catch (err) {
...
router.get('/settlements', (_req, res) =>
  res.sendFile(path.join(__dirname, '..', '..', 'public', 'admin', 'settlements.html')),
);
