  ...
  try {
    ...
    if (type === 'group') {
      [rows] = await pool.query<any[]>(
        `SELECT cr.id, cr.name, cr.is_dm, cr.avatar_url, cr.created_at,
                u.avatar_url AS creator_avatar_url,
                m.last_read_at,
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [result] = await conn.query<any>(
      'INSERT INTO chat_rooms (name, description, avatar_url, created_by) VALUES (?, ?, ?, ?)',
      [name, description ?? null, avatarUrl ?? null, createdBy],
    );
    ...
    const [rows] = await conn.query<any[]>('SELECT * FROM chat_rooms WHERE id = ?', [roomId]);
    res.status(201).json({
      id: rows[0].id,
      name: rows[0].name,
...
router.post('/dm', async (req: Request, res: Response) => {
  ...
  if (existing.length > 0) return res.json({ roomId: existing[0].id });
  ...
  try {
    await conn.beginTransaction();
    const [result] = await conn.query<any>(
      'INSERT INTO chat_rooms (name, is_dm, created_by) VALUES (?, 1, ?)',
      ['dm', userId],
    );
    const roomId = result.insertId;
...
router.get('/:id/members', async (req: Request, res: Response) => {
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT crm.user_id, u.nickname, u.avatar_url, cr.created_by
...
router.post('/:id/join', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'already_joined' });
