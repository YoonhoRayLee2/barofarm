...
import { deleteRoom, endAuction } from '../services/livekit-service';
...
router.patch('/:id/start', async (req: Request, res: Response) => {
  try {
    ...
    startTimer(String(req.params.id), io, async (state) => {
...
router.patch('/:id/end', async (req: Request, res: Response) => {
  try {
    ...
    await deleteRoom(String(req.params.id)); // fire-and-forget (내부에서 에러 삼킴)
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: (e as Error).message }); }
});
